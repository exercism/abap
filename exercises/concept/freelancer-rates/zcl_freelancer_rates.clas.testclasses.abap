CLASS ltcl_freelancer_rates DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  ...
  CLASS ltcl_freelancer_rates IMPLEMENTATION.
    ...
    METHOD test_daysinbudget_1280_16.
      cl_abap_unit_assert=>assert_equals(
      act = cut->days_in_budget( budget  = 1280 rate_per_hour = 16 )
    exp = 10 ).
    ENDMETHOD.
    ...
    METHOD test_discount_16_70_0.
      cl_abap_unit_assert=>assert_equals(
    act = cut->price_with_monthly_discount( rate_per_hour = 16 num_days = 70 discount = 0 )
  exp = 8960 ).
    ENDMETHOD.
  
    METHOD test_discount_16_130_15.
      cl_abap_unit_assert=>assert_equals(
    act = cut->price_with_monthly_discount( rate_per_hour = 16 num_days = 130 discount = '0.15' )
  exp = 14528 ).
    ENDMETHOD.
  
    METHOD test_discount_29_220_112.
      cl_abap_unit_assert=>assert_equals(
    act = cut->price_with_monthly_discount( rate_per_hour = '29.654321' num_days = 220 discount = '0.112' )
  exp = 46346 ).
    ENDMETHOD.
  
    METHOD test_discount_29_155_25.
      cl_abap_unit_assert=>assert_equals(
    act = cut->price_with_monthly_discount( rate_per_hour = '29.654321' num_days = 155 discount = '0.2547' )
  exp = 27466 ).
    ENDMETHOD.
