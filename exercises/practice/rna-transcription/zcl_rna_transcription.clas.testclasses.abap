...
CLASS ltcl_rna_transcription DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      ...
      rna_complement_of_adenine_is_uracil   FOR TESTING,
...
CLASS ltcl_rna_transcription IMPLEMENTATION.
  ...
  METHOD rna_complement_of_cytosine_is_guanine.
    cl_abap_unit_assert=>assert_equals(
      exp = 'G'
      act = cut->transcribe( 'C' ) ).
  ENDMETHOD.

  METHOD rna_complement_of_guanine_is_cytosine.
    cl_abap_unit_assert=>assert_equals(
      exp = 'C'
      act = cut->transcribe( 'G' ) ).
  ENDMETHOD.

  METHOD rna_complement_of_thymine_is_adenine.
    cl_abap_unit_assert=>assert_equals(
      exp = 'A'
      act = cut->transcribe( 'T' ) ).
  ENDMETHOD.

  METHOD rna_complement_of_adenine_is_uracil.
    cl_abap_unit_assert=>assert_equals(
      exp = 'U'
