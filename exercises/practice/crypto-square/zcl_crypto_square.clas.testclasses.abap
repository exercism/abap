...
CLASS ltcl_crypto_square IMPLEMENTATION.
  ...
  METHOD test_single_lowercase_w_space.
    cl_abap_unit_assert=>assert_equals( exp = `u`
                                        act = cut->encode( ` U ` ) ).
  ENDMETHOD.
  ...
  METHOD test_9char_3by3.
    cl_abap_unit_assert=>assert_equals( exp = `tsf hiu isn`
                                        act = cut->encode( `This is fun!` ) ).
  ENDMETHOD.

  METHOD test_8char_3by3.
    cl_abap_unit_assert=>assert_equals( exp = `tso rtu uy `
  ...
  METHOD test_54char_7by8.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `imtgdvs fearwer mayoogo anouuio ntnnlvt wttddes aohghn  sseoau `
